...
def calc_graph(data_frame, output_path,  colors_hdd, colors_cdd, cities, scenarios):
    ...
    fig = tools.make_subplots(rows=2, cols=len(cities), shared_yaxes=True, shared_xaxes=True,
    ...
    for i, city in enumerate(cities):
        ...
        for j, scenario in enumerate(scenarios):
            ...
            scenario_change_0_50.append((data2.loc["2050", "CDD_18_5_C"] - 0)/5)
            ...
            scenario_change_0_50_h.append((data2.loc["2050", "HDD_18_5_C"] - 0)/5)
    ...
    fig['layout'].update(yaxis=dict(domain=[0.5, 1.0]), yaxis2=dict(domain=[0, 0.5]), plot_bgcolor= "rgb(236,243,247)",
                         font=dict(family='Helvetica, monospace', size=18)
                         )
    plot(fig, auto_open=False, filename=output_path + '//' + "lines" + ".html")
    py.image.save_as(fig,filename=output_path + '//' + "lines" + ".png")
